...
import {
  MenuRoleAllocationGroupList,
  MenuRoleAllocationGroupDelete,
  MenuRoleAllocationGroupCreate,
  MenuRoleAllocationAvailableGroupList
} from "@/services/permissions/menu/menu";
import {
  checkUserCreatePermissions,
  checkUserDeletePermissions,
  permissionsMenuRoleAllocationGroupMenuKeys
} from "@/access";
...
export default () => {
  ...
  const columns: ProColumns<MenuRoleAllocationResponse.GroupInfo>[] = [
    ...
    },
    {
      ...
      valueType: 'option',
      render: (dom, record) => [
        checkUserDeletePermissions(permissionsMenuRoleAllocationGroupMenuKeys, userMenuPermissions) &&
  ...
  const ProTableParams: CommonTable.Params = {
    columns: columns,
    permissionsMenuKeys: permissionsMenuRoleAllocationGroupMenuKeys,
    // @ts-ignore
    listRequest: MenuRoleAllocationGroupList,
    // @ts-ignore
    ...
    // @ts-ignore
    toolBarRender: [
      checkUserCreatePermissions(permissionsMenuRoleAllocationGroupMenuKeys, userMenuPermissions) &&
      <Button
        key={"create"}
        type={"primary"}
    ...
    operate: {
      delete: {
        ...
        // @ts-ignore
        request: MenuRoleAllocationGroupDelete
      }
    }
  }
  ...
  const ProModalCreateParams: ProModal.Params = {
    ...
    formItems: [
      {
        ...
        options: {
          request: async () => {
            // @ts-ignore
            const result = await MenuRoleAllocationAvailableGroupList(p?.roleId)
            if (!result.success) {
    ...
    // @ts-ignore
    request: MenuRoleAllocationGroupCreate,
