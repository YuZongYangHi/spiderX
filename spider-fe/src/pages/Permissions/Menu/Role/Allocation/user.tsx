...
import {
  MenuRoleAllocationUserList,
  MenuRoleAllocationUserDelete,
  MenuRoleAllocationUserCreate,
  MenuRoleAllocationAvailableUserList
} from "@/services/permissions/menu/menu";
import {
  checkUserCreatePermissions,
  checkUserDeletePermissions,
  permissionsMenuRoleAllocationUserMenuKeys
} from "@/access";
...
export default () => {
  ...
  const columns: ProColumns<MenuRoleAllocationResponse.UserInfo>[] = [
    ...
    },
    {
      ...
      valueType: 'option',
      render: (dom, record) => [
        checkUserDeletePermissions(permissionsMenuRoleAllocationUserMenuKeys, userMenuPermissions) &&
  ...
  const ProTableParams: CommonTable.Params = {
    columns: columns,
    permissionsMenuKeys: permissionsMenuRoleAllocationUserMenuKeys,
    // @ts-ignore
    listRequest: MenuRoleAllocationUserList,
    // @ts-ignore
    ...
    // @ts-ignore
    toolBarRender: [
      checkUserCreatePermissions(permissionsMenuRoleAllocationUserMenuKeys, userMenuPermissions) &&
      <Button
        key={"create"}
        type={"primary"}
    ...
    operate: {
      delete: {
        ...
        // @ts-ignore
        request: MenuRoleAllocationUserDelete
      }
    }
  }
    ...
    formItems: [
      {
        ...
        options: {
          request: async () => {
            // @ts-ignore
            const result = await MenuRoleAllocationAvailableUserList(p?.roleId)
            if (!result.success) {
        ...
        rules: {
          required: true,
          message: intl.formatMessage({id: 'pages.permissions.menu.list.role.grant.group.columns.userName'})
        }
      },
    ],
    handleOnCancel :handleOnUpdateCancel,
    // @ts-ignore
    request: MenuRoleAllocationUserCreate,
