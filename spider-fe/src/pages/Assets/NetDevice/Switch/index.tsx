...
import {
  ...
  updateSwitch,
  querySwitchList
} from "@/services/Assets/NetDevice/Switch/api";
...
import {
  ...
  assetsNetDeviceSwitchPermissionsMenuKeys
} from "@/access";
...
export default () => {
  ...
  const columns: ProColumns[] = [
    ...
    {
      title: intl.formatMessage({id: "assets.netDevice.switch.column.type"}),
      dataIndex: "type",
      valueType: 'select',
      valueEnum: ColumnConvert["assets.netDevice.switch.type"],
      choices: (value: number, record: any) => ColumnConvert["assets.netDevice.switch.type"][record.type].text,
      render: (_, record) => <Tag color="#108ee9">{ColumnConvert["assets.netDevice.switch.type"][record.type].text}</Tag>
    },
    {
    ...
    {
      title: intl.formatMessage({id: "assets.netDevice.switch.column.mutualRelIp"}),
      dataIndex: "mutualRelIp",
      hideInSearch: true
    },
    {
      title: intl.formatMessage({id: "assets.netDevice.switch.column.upRelPort"}),
      dataIndex: "upRelPort",
      hideInSearch: true
    },
    {
      title: intl.formatMessage({id: "assets.netDevice.switch.column.upIpRelPort"}),
      dataIndex: "upIpRelPort",
      hideInSearch: true
    },
    {
    ...
    },
    {
      ...
      width: 150,
      render: (dom, record) => [
        checkUserUpdatePermissions(assetsNetDeviceSwitchPermissionsMenuKeys, userMenuPermissions) &&
        ...
        checkUserDeletePermissions(assetsNetDeviceSwitchPermissionsMenuKeys, userMenuPermissions) &&
        <span style={clickExtender}
              key="delete"
  ...
  const fetchParams: fetchParamsType = {
    fetch: querySwitchList,
    requestQueryFieldOptions: ["status", "nodeId", "sn", "rackSlotId", "factoryId", "ipNetId", "type", "name"],
    requestParams: []
  }
  ...
  const toolBarRender: ToolBarProps<any>['toolBarRender'] = [
    checkUserCreatePermissions(assetsNetDeviceSwitchPermissionsMenuKeys, userMenuPermissions) && <Button type='primary' onClick={()=>{proModalCreateRef.current?.proModalHandleOpen?.(true)}} >{intl.formatMessage({id: 'assets.node.toolBar.create'})}</Button>,
  ]

  const batchExport: DesignProTable.batchExportT = {
    status: true,
    fileName: "assets.netDevice.switch.export.fileName",
    sheetName: "assets.netDevice.switch.export.sheetName"
  }

  const TableProps: DesignProTable.T = {
    bread: true,
    columns: columns,
    permissionsMenuKeys: assetsNetDeviceSwitchPermissionsMenuKeys,
    fetchParams: fetchParams,
    columnsState: columnsState,
    toolBarRender: toolBarRender,
    batchExport: batchExport,
    deleteRequest: deleteSwitch,
    defaultSize: 'small'
  }

  const ProModalUpdateParams: ProModal.Params = {
    title: 'assets.netDevice.switch.update.title',
    handleOnCancel :handleOnUpdateCancel,
    width: "510px",
    // @ts-ignore
    request: updateSwitch,
  ...
  const ProModalCreateParams: ProModal.Params = {
    title: 'assets.netDevice.switch.create.title',
    width: "510px",
    // @ts-ignore
    params: [],
    initialValues: {},
