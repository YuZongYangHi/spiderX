...
import {
  ...
  permissionsIdcSuitSeasonMenuKeys
} from "@/access";
...
export default () => {
  ...
  const columns: ProColumns[] = [
    ...
    {
      title: intl.formatMessage({id: "idc.suit.season.column.name"}),
      dataIndex: "name",
    },
    {
    ...
    },
    {
      ...
      width: 150,
      render: (dom, record) => [
        checkUserUpdatePermissions(permissionsIdcSuitSeasonMenuKeys, userMenuPermissions) &&
        ...
        checkUserDeletePermissions(permissionsIdcSuitSeasonMenuKeys, userMenuPermissions) &&
        <span style={clickExtender}
              key="delete"
  ...
  const toolBarRender: ToolBarProps<any>['toolBarRender'] = [
    checkUserCreatePermissions(permissionsIdcSuitSeasonMenuKeys, userMenuPermissions) && <Button type='primary' onClick={()=>{proModalCreateRef.current?.proModalHandleOpen?.(true)}} >{intl.formatMessage({id: 'assets.node.toolBar.create'})}</Button>,
  ]

  const batchExport: DesignProTable.batchExportT = {
    status: true,
    fileName: "idc.suit.season.export.fileName",
    sheetName: "idc.suit.season.export.sheetName"
  }

  const TableProps: DesignProTable.T = {
    columns: columns,
    permissionsMenuKeys: permissionsIdcSuitSeasonMenuKeys,
    fetchParams: fetchParams,
    columnsState: columnsState,
    toolBarRender: toolBarRender,
    batchExport: batchExport,
    deleteRequest: deleteIdcSuitSeason,
    defaultSize: 'small'
  }

  const ProModalUpdateParams: ProModal.Params = {
    title: 'idc.suit.season.update.title',
    handleOnCancel :handleOnUpdateCancel,
    width: "510px",
    // @ts-ignore
    request: updateIdcSuitSeason,
  ...
  const ProModalCreateParams: ProModal.Params = {
    title: 'idc.suit.season.create.title',
    width: "510px",
    // @ts-ignore
    params: [],
    initialValues: {},
